...
def make_prompt(language,code):
    output = "For the following %s code, provide a concise docstring summarizing the code.\n%s"%(language,code)

    return output

def run_codexglue(target_dataset, llm_name):
...
try:
    ...
    target_dataset = "/tmp/codexglue/run0/dataset.json"
    llm_name = "gpt-3.5-turbo"

run_codexglue(target_dataset, llm_name)
