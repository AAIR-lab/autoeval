...
def make_prompt(premises,conclusion):
    output = "For the following [PREMISES] containing rules of logical reasoning, perform step-by-step reasoning to answer whether the [CONCLUSION] is True/False/Uncertain based on the [PREMISES]. Use the following answer format:\n Reasoning Steps:\n Answer: True/False/Uncertain \n [PREMISES]:\n%s\n [CONCLUSION]:\n%s\n" % (premises,conclusion)
    return output
...
def run_folio(target_dataset, llm_name):
    ...
    for idx, data in enumerate(dataset['data']):
        messages.append(make_prompt(data["premises"],data["conclusion"]))
